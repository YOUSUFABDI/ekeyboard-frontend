...
import { ApiErrorResponseDT, ApiSuccessResponseDT, OrderHistoryDT, UserDT } from '../../../lib/types'
...
import { useGetOrderHistoryQuery } from '../../../store/order/OrderApi'
        ...
        <div className="">
            <div>
                <ul className="sm:flex font-semibold border-b border-[#ebedf2] dark:border-[#191e3a] mb-5 whitespace-nowrap overflow-y-auto">
                    ...
                    <li className="inline-block">
                        <button
                            onClick={() => toggleTabs('order')}
                            className={`flex gap-2 p-4 border-b border-transparent hover:border-primary hover:text-primary ${tabs === 'order' ? '!border-primary text-primary' : ''}`}
                        >
                            <IconShoppingBag />
                            Order history
                        </button>
...
const OrderHistory = () => {
    const { data } = useGetOrderHistoryQuery()

    const orders = data?.payload?.data ?? []

    return (
        <div className="panel bg-white dark:bg-black border border-[#ebedf2] dark:border-[#191e3a] rounded-md p-4">
            <h6 className="text-lg font-bold mb-5">Order History</h6>
            {orders.length > 0 ? (
                        ...
                        <tbody>
                            {orders.map((order: OrderHistoryDT) => (
                                <tr key={order.orderId} className="hover:bg-gray-50 dark:hover:bg-gray-700">
                                    ...
                                    <td className="p-4 border-b">{new Date(order.createdDate).toLocaleString()}</td>
                                </tr>
                            ))}
                        </tbody>
                    </table>
                ...
                <div className="text-gray-500 dark:text-gray-400 text-center">No order history available.</div>
            )}
        </div>
    )
}
